...
export async function GET(req: NextRequest) {
  ...
  const blockersRootCause = blockedTasks.map((t: any) => ({
    task: t.title,
    blocker: t.blocker || "No blocker details",
    agent: agents.find((a: any) => a.id === t.assigned_agent_id)?.name || "unassigned",
  }));
  ...
  const report = {
    ...
    issues_list: blockersRootCause.map((b: any) => `[${b.agent}] ${b.task}: ${b.blocker}`),
